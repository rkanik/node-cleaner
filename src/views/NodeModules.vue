<template>
  <v-container>
    <v-toolbar flat color="transparent">
      ...
      <v-spacer></v-spacer>
      <v-btn small depressed class="mr-3" color="success" @click="sendTest"
        >TEST</v-btn
      >
      <v-btn
        small
    ...
    >
      <template #item="{item, on, attrs}">
        <v-list-item v-on="on" v-bind="attrs" two-line dense>
          <v-list-item-avatar>
            <v-icon dark class="grey lighten-1">
              mdi-folder
            </v-icon>
          </v-list-item-avatar>
          <v-list-item-content>
    ...
    <div class="px-4 caption truncate" v-if="currentSearchDir">
      <v-progress-circular indeterminate color="primary" size="12" width="1" />
      {{ currentSearchDir }}
    </div>
    <v-toolbar flat dense class="mx-4" v-if="selectedNodeModules.length">
      <v-btn small depressed color="error" @click="selectAll">
        <v-icon left>mdi-check</v-icon>
        Select All
      </v-btn>
      <v-btn small depressed color="error" @click="handleBulkDeleteNodeModules">
        <v-icon left>mdi-delete</v-icon>
        Delete selected
      </v-btn>
    </v-toolbar>
    <v-list dense class="px-4" color="transparent">
      <v-list-item
        two-line
        :key="dirIndex"
        :class="[
          dir.isSelected ? 'primary lighten-4' : 'white',
          { 'mt-1': dirIndex },
        ]"
      ...
      >
        ...
        >
          ...
          />
          <v-icon v-else dark color="primary">mdi-folder</v-icon>
        </v-list-item-avatar>
        <v-list-item-content>
          ...
          <v-menu transition="slide-x-transition" bottom right>
            <template v-slot:activator="{ on, attrs }">
              <v-btn icon v-bind="attrs" v-on="on">
                <v-icon size="20" color="grey" class="text--darken-1"
...
export default {
  ...
  created() {
    ...
    this.$ipc.on(EACH_DIR, (dir) => {
      this.currentSearchDir = dir;
    });
  ...
  methods: {
    selectAll() {
      this.nodeModules = this.nodeModules.map((v) => ({
        ...v,
        isSelected: true,
      }));
    },
    ...
    toggleSelectNodeModule(index) {
      if (this.selectedNodeModulesIndexes.includes(index))
        this.selectedNodeModulesIndexes = this.selectedNodeModulesIndexes.filter(
          (i) => i !== index
        );
